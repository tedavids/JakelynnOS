...
bool initSlab4Page(struct slabAlloc4_t **root) {
    ...
    uint32_t address = (uint32_t) allocVirtKrnlMem(true);
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc4_t *) address;
...
bool initSlab8Page(struct slabAlloc8_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc8_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab16Page(struct slabAlloc16_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc16_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab32Page(struct slabAlloc32_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc32_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab64Page(struct slabAlloc64_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc64_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab128Page(struct slabAlloc128_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc128_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab256Page(struct slabAlloc256_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc256_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab512Page(struct slabAlloc512_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc512_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab1024Page(struct slabAlloc1024_t **root) {
    ...
    // allocate a page
    if (!address) return false;
    *root = (struct slabAlloc1024_t *) address;
    // check for out of memory
    if (!root) return false;
...
bool initSlab2048Page(struct slabAlloc2048_t **root) {
    ...
    uint32_t address = (uint32_t) allocVirtKrnlMem(true);
    // allocate a page, check for out of memory
    if (!address) return false;
    *root = (struct slabAlloc2048_t *) address;
    // check for out of memory
    if (!root) return false;
        ...
        if (slab->address) {
            ...
            if (slab->address == pageaddr) {
                ...
                if (slab->avail == ALLOCSPERPAGE4) {
                    if (deallocVirtKrnlMem(slab->address,true)) {
        ...
        if (curslab->avail > 0) {
            ...
            for (size_t i = 0; i < ALLOCSPERPAGE4; i++) {
                ...
                if (bitval) {
                    ...
                    if (!curslab->address) curslab->address = (uint32_t) allocVirtKrnlMem(true);
                    // check for out of memory
